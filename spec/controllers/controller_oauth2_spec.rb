...
describe SorceryController, active_record: true, type: :controller do
  ...
  context 'with OAuth features' do
    ...
    %i[github google liveid vk salesforce paypal slack wechat microsoft instagram auth0 discord intercom].each do |provider|
  ...
  describe 'OAuth with User Activation features' do
    before(:all) do
      ...
      sorcery_controller_external_property_set(:intercom, :key, 'eYVNBjBDi33aa9GkA3w')
      sorcery_controller_external_property_set(:intercom, :secret, 'XpbeSdCoaKSmQGSeokz5qcUATClRW5u08QWNfv71N8')
      sorcery_controller_external_property_set(:intercom, :callback_url, 'http://blabla.com')
    end
    ...
    %i[github google liveid vk salesforce paypal wechat microsoft instagram auth0 discord intercom].each do |provider|
  ...
  describe 'OAuth with user activation features' do
    ...
    %w[facebook github google liveid vk salesforce slack discord intercom].each do |provider|
  ...
  def provider_url(provider)
    {
      ...
      intercom: "https://app.intercom.com/oauth?client_id=#{::Sorcery::Controller::Config.intercom.key}&display&redirect_uri=http%3A%2F%2Fblabla.com&response_type=code&scope&state"
    }[provider]
  end
end
